...
positpal::start("Group_6_CC2.Rmd")
```
...
Coding Challenge 1 dataset
-------------------------------
variable | description | units
---------|-------------|---------
Population Type | asexual or sexual population | no unit
Ovary Width | width of a given plant's ovary | mm
Ovary Height | height of a given plant's ovary | mm
...
```{r import-clean}
cc1_data1 <- read.csv("data/A1_Data_Excel.csv")
cc1_data <- cc1_data1[1:46, 1:8]

cc1_data$ovary_area <- cc1_data$ovary_width * 
    cc1_data$ovary_height
...
kable(area_means, 
        col.names = c("Population Type", "Mean Ovary Area"), 
        caption = "Mean Ovary Area by Population Type")
```
...
ggplot(cc1_data, aes(x=ovary_area)) + 
    ...
    labs(
        title = "Distribution of Ovary Area by Population Type", 
        x = "Ovary Area (mm²)",
        y = "Frequency"
    ) +
    theme_classic()
...
![*Solanum pimpinellifolium*, or currant tomato is a wild species of tomato native to Ecuador and Peru](./images/Solanum_pimpinellifolium_-_C_1.jpg)

Source: [Little Red Wagon Nursery](https://littleredwagonnativenursery.com/products/everglades-tomato-solanum-pimpinellifolium)
